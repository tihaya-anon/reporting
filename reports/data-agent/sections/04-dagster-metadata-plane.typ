...
Dagster 提供一层浅层元数据。它不替代 AWS，而是给 agent 一个“指针索引”：先知道哪个 asset、run、code location、sensor 或 ECS task 失败，再决定查哪条 AWS 证据链。

DataAgent 的 ETL 主要通过 API 拉取数据，不是 CDC 链路。因此 Python `dlt` package 也要进入调试模型。`dlt` 运行时产生 load、schema、resource、state 等元数据，可以和 Dagster run / asset metadata 一起作为排障入口。
...
日志中的 Prefect 清理、Dagster stack、asset dependency boundary、ODS source assets、sensors、EFS release、code location metadata、pool limits、resource concurrency key，都可以放进这张运行地图。Dagster 负责 run、asset 和 code location；`dlt` 补上 API 拉取型 ETL 的 load、schema、resource 和 state。agent 不必只从最终表或 CloudWatch 日志倒推问题。
